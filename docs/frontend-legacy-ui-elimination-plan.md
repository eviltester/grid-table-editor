...
- `packages/core-ui/js/gui_components/generator/runtime/create-generator-page.js`
  - The remaining live generator page seam is now the direct page factory plus its surviving page-service cluster, not the older shell/facade stack. Page creation flows through `create-generator-page.js`, orchestration lives in `generator-page-service.js`, mounted page assembly flows through `create-generator-page-runtime-mount.js` plus `create-generator-page-runtime-config.js`, and the live page-service path now uses responsibility-based names such as `create-generator-page-services.js`, `generator-page-actions-service.js`, `generator-page-view-state.js`, and `create-generator-page-schema-services.js`. The main remaining cleanup is narrower follow-up regrouping only if the surviving schema-generation helper or page orchestration still feels broader than one clear page boundary.
