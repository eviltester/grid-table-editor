...
The generator runtime entry is simpler now. `data-generator-page-runtime.js` delegates directly to `create-generator-runtime.js`, and mounted page assembly flows through `create-generator-page-runtime-mount.js` plus `create-generator-page-runtime-config.js`. The remaining generator cleanup work is no longer the old shell/facade/config pyramid; it is the narrower follow-on simplification of the surviving action/view-state/schema helper clusters under `generator/runtime/`, as tracked in `docs/generator-runtime-simplification-map.md` and `docs/frontend-mvc-cleanliness-checklist.md`.
...
- `ImportExportWorkspace` now delegates its async import/export orchestration into `create-import-export-workspace-runtime.js`, so the public component entrypoint is mostly root/document resolution plus runtime delegation rather than acting as the main feature brain itself.
