...
defineDomainKeywordHelpContractTests('string.binary');

describe('string.binary parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('string.binary');
    const parsed = parseKeywordInvocation(`string.binary(${paramsText})`);
  ...
  test('executes string.binary(length=5) successfully', () => {
    const parsed = parseKeywordInvocation('string.binary(length=5)');
  ...
  test('rejects invalid prefix type before generation', () => {
    expect(validateArgs('length=5, prefix={"bad":true}')).toEqual({
      ok: false,
