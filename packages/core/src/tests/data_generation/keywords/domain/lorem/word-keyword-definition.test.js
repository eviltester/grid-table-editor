...
defineDomainKeywordHelpContractTests('lorem.word');

describe('lorem.word parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('lorem.word');
    const parsed = parseKeywordInvocation(`lorem.word(${paramsText})`);
  ...
  test('executes lorem.word(length=5) successfully', () => {
    const parsed = parseKeywordInvocation('lorem.word(length=5)');
