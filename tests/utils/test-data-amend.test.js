...
import { TEST_DATA_MODES, createAmendedTable, createTableFromGenerator } from "../../js/gui_components/test-data-amend.js";
...
describe("test-data amend behavior", () => {
    test("new table mode generates exact row count", () => {
        const generator = createGenerator(["A", "B"], [["1", "2"], ["3", "4"]]);
    ...
    test("amend table updates schema columns, preserves others, and appends rows when needed", () => {
        ...
        const generator = createGenerator(["name", "newcol"], [
            ["new-1", "n1"],
            ["new-2", "n2"],
            ["new-3", "n3"]
        ]);
