...
defineDomainKeywordHelpContractTests('number.hex');

describe('number.hex parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('number.hex');
    const parsed = parseKeywordInvocation(`number.hex(${paramsText})`);
  ...
  test('executes number.hex(max=10, min=1) successfully', () => {
    const parsed = parseKeywordInvocation('number.hex(max=10, min=1)');
