...
defineDomainKeywordHelpContractTests('lorem.slug');

describe('lorem.slug parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('lorem.slug');
    const parsed = parseKeywordInvocation(`lorem.slug(${paramsText})`);
  ...
  test('executes lorem.slug(max=10, min=1) successfully', () => {
    const parsed = parseKeywordInvocation('lorem.slug(max=10, min=1)');
