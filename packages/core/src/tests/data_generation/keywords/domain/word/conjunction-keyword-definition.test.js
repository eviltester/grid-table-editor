...
defineDomainKeywordHelpContractTests('word.conjunction');

describe('word.conjunction parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('word.conjunction');
    const parsed = parseKeywordInvocation(`word.conjunction(${paramsText})`);
  ...
  test('executes word.conjunction(length=5) successfully', () => {
    const parsed = parseKeywordInvocation('word.conjunction(length=5)');
