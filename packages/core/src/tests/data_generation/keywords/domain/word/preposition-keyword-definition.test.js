...
defineDomainKeywordHelpContractTests('word.preposition');

describe('word.preposition parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('word.preposition');
    const parsed = parseKeywordInvocation(`word.preposition(${paramsText})`);
  ...
  test('executes word.preposition(length=5) successfully', () => {
    const parsed = parseKeywordInvocation('word.preposition(length=5)');
