...
Page shell components own reusable functional scaffold only: instruction roots, feature mount roots, and any page-internal layout needed by those features. The app shell owns the native `details` wrappers around Test Data and Import / Export while keeping the feature mount roots inside those wrappers. Host-app chrome such as site headers, navigation, analytics tags, and startup loading placeholders belongs in the app entry HTML or host runtime, not in `core-ui` shells. Shells should not reach into feature internals.
