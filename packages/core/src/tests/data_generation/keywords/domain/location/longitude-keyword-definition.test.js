...
defineDomainKeywordHelpContractTests('location.longitude');

describe('location.longitude parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('location.longitude');
    const parsed = parseKeywordInvocation(`location.longitude(${paramsText})`);
  ...
  test('executes location.longitude(max=10, min=1) successfully', () => {
    const parsed = parseKeywordInvocation('location.longitude(max=10, min=1)');
