...
defineDomainKeywordHelpContractTests('lorem.sentences');

describe('lorem.sentences parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('lorem.sentences');
    const parsed = parseKeywordInvocation(`lorem.sentences(${paramsText})`);
  ...
  test('executes lorem.sentences(max=10, min=1) successfully', () => {
    const parsed = parseKeywordInvocation('lorem.sentences(max=10, min=1)');
