...
defineDomainKeywordHelpContractTests('lorem.sentence');

describe('lorem.sentence parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('lorem.sentence');
    const parsed = parseKeywordInvocation(`lorem.sentence(${paramsText})`);
  ...
  test('executes lorem.sentence(max=10, min=1) successfully', () => {
    const parsed = parseKeywordInvocation('lorem.sentence(max=10, min=1)');
  ...
  test('rejects mixed generic range and specific count before generation', () => {
    expect(validateArgs('min=1, max=10, wordCount=5')).toEqual({
      ok: false,
      error:
        'Invalid keyword arguments: use only one lorem count scheme: min/max, wordCount, or wordCountMin/wordCountMax',
    });
  });

  test('rejects mixed generic range and specific count range before generation', () => {
    expect(validateArgs('min=1, max=10, wordCountMin=2, wordCountMax=5')).toEqual({
      ok: false,
      error:
        'Invalid keyword arguments: use only one lorem count scheme: min/max, wordCount, or wordCountMin/wordCountMax',
    });
