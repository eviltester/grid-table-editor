...
This plan defines how to replace ad hoc text, Unicode, AG Grid, and one-off legacy SVG icons with one consistent icon approach across the frontend UI. The implemented target is Lucide-style inline SVG icons, delivered through a small shared helper so plain JavaScript components can render icons without adopting a framework.

## Goals

- Standardize action and help icons across Tabulator, AG Grid, shared controls, and option panels.
...
- Avoid new grid-specific icon dependencies or reliance on AG Grid's built-in icon classes for custom app actions.
...
- Tabulator column header action text appears in `packages/core-ui/js/gui_components/data-grid-editor/tabulator/customHeader-tabulator.js`.
...
- AG Grid column header action text appears in `packages/core-ui/js/gui_components/data-grid-editor/ag-grid/customHeader-ag-grid.js`.
...
- Several shared controls use text or Unicode symbols, including schema row actions in `packages/core-ui/js/gui_components/generator/schema/data-generator-schema-ui.js`.
- Some grid chrome still uses AG Grid classes for built-in grid behavior. Those can remain until a separate grid-chrome replacement is intentionally planned.
...
- [x] Replace Tabulator popup header action text in `tabulator/customHeader-tabulator.js`.
- [x] Replace AG Grid custom header action text in `ag-grid/customHeader-ag-grid.js`.
...
- [x] Replace schema row action glyphs in `data-generator-schema-ui.js` with standardized icons.
...
- Layout stability: replacing text with SVGs should not change column header height unexpectedly or cause Tabulator/AG Grid header resize jitter.
...
- Third-party formatters: Tabulator and AG Grid header formatters may have different event and markup constraints, so migrate each with focused tests.
