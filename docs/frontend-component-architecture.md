...
The embedded app test-data feature no longer has extra mounted-entrypoint wrappers either: live runtime code, focused tests, and harnesses now resolve `mountTestDataGenerationPanel(...)` directly from `controller/test-data-grid-controller.js`, and both the old one-export top-level `app/test-data-grid/index.js` pass-through plus the older `controller/index.js` pass-through are removed. UI generation execution is now shared one layer deeper as well: both the embedded app test-data service and the standalone generator page route row generation, pairwise generation, combinations generation, and buffered amend execution through the shared `shared/test-data/generation/ui-generation-session-service.js` boundary, leaving grid apply / preview sync / download behavior as the remaining shell-specific adapters.
...
Page shell components own reusable functional scaffold only: instruction roots, feature mount roots, and any page-internal layout needed by those features. The app shell owns the native `details` wrapper around Test Data while keeping Import / Export as a direct workspace mount root; the import/export workspace owns its own visible grid/preview sync row plus a closed-by-default import/export disclosure internally. Host-app chrome such as site headers, navigation, analytics tags, and startup loading placeholders belongs in the app entry HTML or host runtime, not in `core-ui` shells. Shells should not reach into feature internals.
...
- `ImportExportWorkspace` now composes `ImportExportGridPreviewSyncControl`, `ImportExportToolbar`, `FormatSelector`, `TextPreviewEditor`, and the focused `OptionsPreviewSplitLayout` shell. `ImportExportToolbar` is now a thin visible host around the narrower `ImportExportImportControl` and `ImportExportDownloadControl` MVC components, while the workspace keeps the grid/preview sync row visibly separate above the disclosure. The workspace still owns import/export behavior and format-option panel lifecycles, while the split layout owns only the visible options-versus-preview shell, resizing, and width clamping.
