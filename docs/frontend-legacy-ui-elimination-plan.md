...
- `packages/core-ui/js/gui_components/generator/help/*`
  - Generator-specific schema/help copy that still belongs to the generator host while shared schema behavior continues moving into shared internals.
...
- Phase 3 no longer has an active `generator/schema/data-generator-schema-ui.js` blocker. The old wrapper was deleted after shared row behavior moved out and the remaining schema-mode help copy moved into `generator/help/*`.
...
- The old generator schema wrapper has been deleted. Generator-specific schema-mode help now lives in `packages/core-ui/js/gui_components/generator/help/`, and shared row helper behavior no longer flows through a generator-owned module at all.
- Generator runtime row-count parsing now prefers `GeneratorPreview` / `GeneratorControls` component APIs and only falls back to the legacy page-global `previewRowsCount` / `generateRowsCount` id parser as a compatibility bridge.
...
- `DataGeneratorPage` no longer keeps a separate `selectedOutputType` shadow copy. Current format now comes from `GeneratorControls` state/value, with `csv` only as the unmounted default.
...
- `DataGeneratorPage` no longer stores or orchestrates `formatOptionsPanel` / `optionsPanels`, and the old runtime-only options dirty-state helpers have been removed. The options panel lifecycle is now owned by `GeneratorControls` plus `FormatOptionsPanel`.
...
- Apply-options orchestration now lives in `generator/options/apply-generator-format-options.js`, so `DataGeneratorPage` delegates sanitize/apply/post-apply coordination instead of mixing exporter option mutation, controls resync, preview refresh, and status messaging inline.
...
- Text-mode/schema-sync orchestration now lives in `generator/runtime/generator-schema-sync.js`, so `DataGeneratorPage` delegates text-mode sync, semantic-validation, and schema-error surfacing coordination instead of keeping that flow inline.
...
- Mounted shared schema state is now owned by `SharedSchemaDefinition`; `DataGeneratorPage` no longer mirrors row/token/text-mode writes back into its private fallback schema session after mount, and stale runtime leftovers like `rowCountControls`, `invalidateSchemaTokensFromRows()`, and `handleGlobalButtonClick()` have been removed.
- `DataGeneratorPage` no longer carries a broad set of dead schema passthrough methods for parser access, row actions, drag/drop handlers, or mode-update wrappers when production code already talks directly to `SharedSchemaDefinition`; generator-page coverage now exercises those behaviors through the shared schema feature boundary instead of the page runtime facade.
...
- Shared schema validation-state classes now use shared defaults for invalid rows, invalid fields, and row validation text. Generator-era validation class names remain only as temporary CSS aliases.
- Shared schema row action and command-picker markup now use shared default classes. Generator-era action/command-picker class names remain only as temporary CSS compatibility aliases, not as shared rendered defaults.
- Shared schema row layout and drag/drop state classes now use shared defaults. Generator-era row-mode and drag/drop class names remain only as temporary CSS compatibility aliases, not as shared rendered defaults.
- Shared schema row root markup now renders `shared-schema-row` without the old `generator-schema-row` bridge class. App and generator harnesses/page objects now query the shared row contract directly, while generator-era row selectors remain only as temporary CSS compatibility aliases.
...
- The shared grid error surface no longer depends on the page-global `#grid-column-error` id in its live data-grid component path. `DataGridComponentView` now passes a rooted resolver for `[data-role="grid-error-status"]`, while the old id remains only as a compatibility fallback for callers that still intentionally rely on the page-level anchor.
...
- Confirm dialog service now owns a component-backed dialog instance with explicit `requestConfirm()` and `destroy()` lifecycle, while `showConfirmModal(...)` remains as a compatibility wrapper for existing direct consumers.
- Text-input dialog service now also owns a component-backed dialog instance with explicit `requestTextInput()` and `destroy()` lifecycle, while `showTextInputModal(...)` remains as a compatibility wrapper for existing direct consumers.
...
- `initHelpTooltips(...)` remains as the page-level bootstrap path, but it now returns the same service object and owns the legacy `window.updateHelpHints` compatibility hook only for that bootstrap case.
...
- Theme toggle now exposes a small `createThemeToggleComponent(...)` page-shell feature with explicit `getState()`, `toggleTheme()`, `setTheme()`, and `destroy()` lifecycle. App and generator bootstraps still call `initThemeToggle(...)`, but that function now acts as a compatibility wrapper over the component API and returns a destroyable instance.
...
- The shared theme-toggle feature no longer emits or re-finds a default `#theme-toggle-button` id in its live path. The component now relies on the rooted `data-role="theme-toggle-button"` hook and the visible button contract instead, leaving only the styling classes as presentation markup.
- The shared confirm/text-input modal helpers now resolve their owned title/message/input/button elements through rooted `data-role` hooks inside the overlay subtree rather than fixed child-ID queries. The document-level backdrop and button/input IDs remain as the intentional public dialog-host contract for cleanup and browser/page-object integration.
...
- `GridToolbarView` now uses rooted `data-role` hooks for its private toolbar controls and filter/unique-name wiring, while the older child IDs remain only as page-level compatibility contracts for the app shell and any legacy consumers.
