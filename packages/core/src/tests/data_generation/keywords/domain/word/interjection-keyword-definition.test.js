...
defineDomainKeywordHelpContractTests('word.interjection');

describe('word.interjection parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('word.interjection');
    const parsed = parseKeywordInvocation(`word.interjection(${paramsText})`);
  ...
  test('executes word.interjection(length=5) successfully', () => {
    const parsed = parseKeywordInvocation('word.interjection(length=5)');
