...
- `packages/core-ui/js/gui_components/generator/runtime/data-generator-page-runtime.js`
  - The remaining live generator runtime seam is now mostly the top-level generator runtime lifecycle shell around the mounted/unmounted factory helpers. The runtime entry module is thinner now that base-state/default assembly lives in `create-generator-runtime-base-state.js`, runtime entry parser/sample-schema assembly lives in `create-generator-runtime-factory-inputs.js`, runtime schema-helper adaptation lives in `create-generator-runtime-schema-adapters.js`, uninitialized runtime shell wiring lives in `create-generator-runtime-instance.js`, shell config lives in `create-generator-runtime-shell-config.js`, shell factory composition lives in `create-generator-runtime-factories.js`, runtime-driven mount closure assembly lives in `create-generator-page-runtime-mount-factory.js`, runtime-driven dependency closure assembly lives in `create-generator-runtime-dependencies-factory.js`, the unavailable row-count fallback now lives in `create-generator-unavailable-row-count-result.js`, the shell/dependency attachment layer lives in `create-generator-runtime-shell.js`, lifecycle-backed runtime facade assembly now lives in `create-generator-runtime-lifecycle-facade.js`, runtime core schema-state bridging now lives in `create-generator-runtime-core.js`, runtime facade methods live in `create-generator-runtime-facade.js`, runtime schema-state property bridging now lives in `create-generator-runtime-schema-state.js`, runtime mount precondition checks now live in `assert-generator-runtime-mountable.js`, lifecycle mount/destroy behavior live in `create-generator-runtime-lifecycle.js`, schema catalog/support assembly lives in `create-generator-runtime-schema-support-dependencies.js`, schema editing-session assembly now lives in `create-generator-runtime-schema-session-dependencies.js`, schema runtime/generation/state bridge assembly now lives in `create-generator-runtime-schema-bridges.js`, the remaining schema-side runtime bundle in `create-generator-runtime-schema-dependencies.js` now just composes those support, session, and bridge helpers, mounted view-state bridge assembly now lives in `create-generator-runtime-view-state-dependencies.js`, runtime-action bridge assembly now lives in `create-generator-runtime-actions-dependencies.js`, generator-page runtime-config assembly now lives in `create-generator-page-component-runtime-config.js`, generator-page config-input assembly now lives in `create-generator-page-component-config-inputs.js`, the runtime-owned dependency mapping now lives in `create-generator-page-component-runtime-dependencies.js`, that dependency layer now composes schema-side inputs from `create-generator-page-component-runtime-schema-dependencies.js` plus exporter/grid service handles from `create-generator-page-component-runtime-service-dependencies.js`, the runtime-owned callback mapping now lives in `create-generator-page-component-runtime-callbacks.js`, that callback layer now composes controls, preview, and schema callback bundles from `create-generator-page-component-runtime-controls-callbacks.js`, `create-generator-page-component-runtime-preview-callbacks.js`, and `create-generator-page-component-runtime-schema-callbacks.js`, generator-page config dependency assembly now lives in `create-generator-page-component-config-dependencies.js`, generator-page props assembly now lives in `create-generator-page-component-props.js`, generator-page controls-prop assembly now lives in `create-generator-controls-props.js`, generator-page preview-prop assembly now lives in `create-generator-preview-props.js`, generator-page schema-definition prop assembly now lives in `create-generator-schema-definition-props.js`, generator-page service assembly now lives in `create-generator-page-component-services.js`, generator-page controls-service assembly now lives in `create-generator-controls-services.js`, generator-page preview-service assembly now lives in `create-generator-preview-services.js`, generator-page callback wiring now lives in `create-generator-page-component-callbacks.js`, controls callback wiring now lives in `create-generator-controls-callbacks.js`, preview callback wiring now lives in `create-generator-preview-callbacks.js`, schema-definition callback wiring now lives in `create-generator-schema-definition-callbacks.js`, mounted-page state assembly now lives in `create-generator-mounted-page-state.js`, mounted-page startup sync now lives in `create-generator-mounted-page-startup-sync.js`, the remaining `create-generator-runtime-dependencies.js` layer just composes those runtime sub-bundles, pure schema-rule helpers live in `generator-schema-rule-helpers.js`, mounted feature extraction is isolated in `generator-mounted-page-bridge.js`, and `create-generator-page-runtime-mount.js` is down to page creation plus mounted-state plus startup-sync sequencing on top of those narrower helpers. The runtime entry itself now stays on runtime factories only instead of doubling as the public helper-export barrel. The remaining cleanup is still the overall generator-page runtime composition path rather than a broad bridge or class shell.
  - Generator runtime now also uses the generator-specific example schema as its default sample text. `create-generator-runtime-factory-inputs.js` imports `GENERATOR_DEFAULT_EXAMPLE_SCHEMA_TEXT` directly from `shared/test-data/schema/schema-examples.js`, while the embedded app path keeps `TEST_DATA_GRID_SAMPLE_SCHEMA_TEXT` as its own sample-schema contract.
...
- Configured-generator creation and enum-column counting for generator preview/export flows now live behind a dedicated schema-generation bridge in `generator/generation/generator-schema-generation-bridge.js`, so `DataGeneratorPage` no longer owns that schema-to-generator orchestration method directly.
- Schema text-mode sync, schema revalidation, and schema-vs-generation error routing now live behind `generator/runtime/generator-schema-runtime-bridge.js`, so `DataGeneratorPage` no longer owns that validation/error coordination cluster directly.
...
- Pairwise-visibility calculation itself now lives behind the schema-generation bridge, so `DataGeneratorPage` no longer interprets schema rows directly when deciding whether the pairwise action should be exposed.
...
- The extra top-level `packages/core-ui/js/gui_components/generator/index.js` pass-through barrel is gone. Standalone bootstrap, focused harnesses, and generator runtime coverage now import `createDataGeneratorPage(...)` directly from `generator/runtime/data-generator-page-runtime.js`, while low-level schema-rule helpers continue resolving through `generator-schema-rule-helpers.js` and unmounted runtime assertions use the dedicated runtime helper rather than advertising extra generator internals as part of a broader feature entry surface.
...
- The text-preview Playwright abstraction now also queries `Preview`/`Edit`, `Auto Preview`, and `Copy` through component-scoped role/name selectors, with the format subtasks anchored to the component root instead of old fixed child IDs.
...
- The export-preview Storybook harness no longer wires its action logging through legacy preview child IDs like `#settextfromgridbutton`, `#previewEditModeButton`, `#autoPreviewCheckbox`, or `#copyTextButton`. It now attaches to the mounted workspace through visible button text and the `Auto Preview` label instead.
