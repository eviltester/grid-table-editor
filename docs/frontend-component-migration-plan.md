...
- `SharedSchemaDefinition` now also owns plain-text schema file load/save controls, backed by shared file-transfer services, so the app test-data panel, generator page, and writer-schema page all share the same schema file workflow instead of growing page-specific file handlers.
