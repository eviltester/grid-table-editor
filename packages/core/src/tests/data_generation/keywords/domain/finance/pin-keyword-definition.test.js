...
defineDomainKeywordHelpContractTests('finance.pin');

describe('finance.pin parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('finance.pin');
    const parsed = parseKeywordInvocation(`finance.pin(${paramsText})`);
  ...
  test('executes finance.pin(length=5) successfully', () => {
    const parsed = parseKeywordInvocation('finance.pin(length=5)');
