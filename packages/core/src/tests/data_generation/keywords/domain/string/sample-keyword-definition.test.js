...
defineDomainKeywordHelpContractTests('string.sample');

describe('string.sample parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('string.sample');
    const parsed = parseKeywordInvocation(`string.sample(${paramsText})`);
  ...
  test('executes string.sample(length=5) successfully', () => {
    const parsed = parseKeywordInvocation('string.sample(length=5)');
