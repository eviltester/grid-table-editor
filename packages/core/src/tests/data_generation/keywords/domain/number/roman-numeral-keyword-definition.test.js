...
defineDomainKeywordHelpContractTests('number.romanNumeral');

describe('number.romanNumeral parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('number.romanNumeral');
    const parsed = parseKeywordInvocation(`number.romanNumeral(${paramsText})`);
  ...
  test('executes number.romanNumeral(max=10, min=1) successfully', () => {
    const parsed = parseKeywordInvocation('number.romanNumeral(max=10, min=1)');
