// based on https://raw.githubusercontent.com/AllMightySauron/ascii-table3/main/ascii-table3.js
import { styles } from './ascii-table3-styles.js';
import { strlen, isBlank, partition } from './printable-characters.js';
...
class AsciiTable3 {
  ...
  static align(direction, value, len, pad = ' ') {
    ...
    if (direction == AlignmentEnum.RIGHT) {
      ...
      return AsciiTable3.padEnd(
        AsciiTable3.padStart(strValue, strlen(strValue) + Math.floor((len - strlen(strValue)) / 2), pad),
        len,
        pad
      );
    } else {
  ...
  static truncateString(str, maxSize) {
    ...
    if (strlen(str) > maxSize) {
      ...
      for (const [nonPrintable, printable] of partition(str)) {
        const text = Array.from(printable.substring(0, maxSize - SUFIX.length).concat(SUFIX)).slice(
          0,
          maxSize - length
        );
  ...
  transpose() {
    ...
    // new value
    return new AsciiTable3(this.getTitle()).setHeadingAlign(this.getHeadingAlign()).addRowMatrix(newMatrix);
  }
  ...
  toString() {
    ...
    if (this.getTitle().length > 0) {
      ...
      result +=
        style.borders.top.left + ''.padStart(maxWidth, style.borders.top.center) + style.borders.top.right + '\n';

      if (result.trim() == '') result = '';

      // title line
      result +=
        style.borders.data.left +
        AsciiTable3.align(this.getTitleAlign(), this.getTitle(), maxWidth) +
