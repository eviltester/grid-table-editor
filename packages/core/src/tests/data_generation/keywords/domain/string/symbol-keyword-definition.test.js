...
defineDomainKeywordHelpContractTests('string.symbol');

describe('string.symbol parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('string.symbol');
    const parsed = parseKeywordInvocation(`string.symbol(${paramsText})`);
  ...
  test('executes string.symbol(length=5) successfully', () => {
    const parsed = parseKeywordInvocation('string.symbol(length=5)');
