...
2. `pnpm run ast-grep` — repo-specific structural rules must pass
3. `pnpm test` — all Jest tests must pass
4. `pnpm run test:browser` — Playwright smoke tests must pass

Do not call `task_complete` until all four commands exit successfully.
