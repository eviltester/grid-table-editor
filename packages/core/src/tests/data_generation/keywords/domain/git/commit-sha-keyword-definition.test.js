...
defineDomainKeywordHelpContractTests('git.commitSha');

describe('git.commitSha parameter validation', () => {
  function validateArgs(paramsText) {
    const keyword = getDomainKeywordByAlias('git.commitSha');
    const parsed = parseKeywordInvocation(`git.commitSha(${paramsText})`);
  ...
  test('executes git.commitSha(length=7) successfully', () => {
    const parsed = parseKeywordInvocation('git.commitSha(length=7)');
